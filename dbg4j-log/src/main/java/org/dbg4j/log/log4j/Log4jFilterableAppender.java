...
/**
 * This appender uses a log4j Logger to persist the debugging output. The various constructors
 ...
 */
public class Log4jFilterableAppender extends FilterableAppender implements ContextListener {

    private static final String DEFAULT_NAME = Log4jFilterableAppender.class.getName();
    ...
    public Log4jFilterableAppender() {
        this((Logger) null, null, null);
    }

    public Log4jFilterableAppender(Logger logger) {
        this(logger, null, null);
    }

    public Log4jFilterableAppender(String loggerName) {
        this(loggerName, null, null);
    }

    public Log4jFilterableAppender(Logger logger, Level priority) {
        this(logger, priority, null);
    }

    public Log4jFilterableAppender(String loggerName, Level priority) {
        this(Logger.getLogger(loggerName), priority, null);
    }

    public Log4jFilterableAppender(String loggerName, Level priority, DebugOutputFormatter formatter) {
        this(Logger.getLogger(loggerName), priority, formatter);
    }


    public Log4jFilterableAppender(Logger logger, Level priority, DebugOutputFormatter formatter) {
        if(logger != null) {
            this.logger = logger;
        } else {
     ...
     * {@link Log4jFilterableAppender#setEventType(org.dbg4j.core.context.DebugContext.EventType)})
     *
     * @param eventType
     * @param debugContext
     * @param parameters
